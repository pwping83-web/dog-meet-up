        ...
        <div className="mb-10">
          ...
          <div className="mt-3 flex justify-end">
            <button
              ...
              aria-label="이 글 신고하기"
              title="이 글 신고하기"
              className="rounded-full border border-slate-200/90 bg-white p-1.5 text-rose-500 shadow-sm transition-colors hover:border-rose-200 hover:bg-rose-50 active:scale-95 disabled:opacity-50"
            >
              <Flag className="h-3.5 w-3.5" strokeWidth={2.25} aria-hidden />
            </button>
          </div>
        ...
        {!isCareMeetupCategory(meetup.category) && (
          <div className="mb-10 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="mb-2 flex items-center gap-2">
              ...
              <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-bold text-slate-600 tabular-nums">
                {comments.length}
              </span>
            </div>
            <div className="space-y-2">
              <textarea
                ...
                maxLength={300}
                placeholder="내용 입력"
                className="w-full resize-none rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-medium text-slate-800 placeholder:text-slate-400"
              />
              <button
                type="button"
                onClick={() => void handleCommentSubmit()}
                disabled={authLoading}
                className="w-full rounded-xl bg-slate-900 py-2.5 text-sm font-extrabold text-white transition-opacity active:opacity-90 disabled:opacity-50"
              >
                등록
              </button>
            </div>
            ...
            {comments.length === 0 ? (
              <p className="mt-3 text-center text-[11px] font-medium text-slate-400">아직 댓글이 없어요.</p>
            ) : (

...
> **현재 서비스:** 저장소는 **댕댕마켓**(반려견 모임·돌봄) 기준으로 개발 중입니다. 아래 본문 중 Supabase 예시·테이블명(`repair_requests` 등)은 **옛 스키마**를 담고 있을 수 있으니, **최신 안내는 루트 `README.md`와 `supabase/migrations/`** 를 우선하세요.

## 📦 프로젝트 개요 (구버전 서술)

**구)** 당근마켓 스타일의 지역 기반 중개 플랫폼 초안 문서가 이 파일에 남아 있습니다.
